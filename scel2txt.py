...
def get_records(f, file_size, hz_offset, py_map):
    ...
    while f.tell() != file_size:
        ...
        py_set = []
        for _ in range(py_idx_count):
            py_idx = read_uint16(f)
            if py_map.get(py_idx, None) is None:
                return records
...
def get_words_from_sogou_cell_dict(fname) -> List:
...
def download_sogou_cell_dict(fname) -> None:
